...
public class VideoItemListActivity extends AppCompatActivity
    ...
    //COMP 530
    //shared preferences file name to store isSubscribed and channelID
    public static final String PREFS_NAME = "newpipe_prefs";
    //shared preference key for isSubscribed
    public static final String PREFS_KEY_SUBSCRIBED = "isSubscribed";
    //shared preference key for channel ID
    public static final String PREFS_KEY_CHANNEL_ID = "Channel_Id";
    //Comp 530
    boolean bUploaderVideos = false;
    ...
    private class SearchVideoQueryListener implements SearchView.OnQueryTextListener {
        ...
        public boolean onQueryTextSubmit(String query) {
            try {
                searchQuery = query;
                listFragment.search(query, bUploaderVideos);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState != null
            ...
            if(!searchQuery.isEmpty()) {
                listFragment.search(searchQuery, bUploaderVideos);
            }
        }
       ...
       //COMP 530
        // if the user subscribed to a channel, load channel videos
        SharedPreferences shp = getSharedPreferences(VideoItemListActivity.PREFS_NAME, Context.MODE_PRIVATE);
        isSubscribed = shp.getBoolean(VideoItemListActivity.PREFS_KEY_SUBSCRIBED, false);
        if(isSubscribed) {
            String channelId = shp.getString(VideoItemListActivity.PREFS_KEY_CHANNEL_ID, "");
            if(!channelId.equalsIgnoreCase("")) {
                // end point URL to get channel videos
                searchQuery = "https://www.youtube.com/channel/"+channelId+"/videos";
                listFragment.search(searchQuery, true);
                //to make NewPipe view invisible
                bg.setVisibility(View.GONE);
            }
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if(mode != PRESENT_VIDEOS_MODE &&
            ...
            searchView.setSuggestionsAdapter(suggestionListAdapter);
            searchView.setOnSuggestionListener(new SearchSuggestionListener(searchView));
            if(!searchQuery.isEmpty() && !isSubscribed) {
                searchView.setQuery(searchQuery,false);
                searchView.setIconifiedByDefault(false);
            }
